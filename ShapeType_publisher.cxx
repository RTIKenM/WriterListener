...
extern "C" int publisher_main(int domainId, int sample_count)
{
    ...
    ShapeTypeExtended *instanceA = NULL;
	ShapeTypeExtended *instanceB = NULL;
	DDS_ReturnCode_t retcode;
    DDS_InstanceHandle_t instanceA_handle = DDS_HANDLE_NIL;
	DDS_InstanceHandle_t instanceB_handle = DDS_HANDLE_NIL;
	...
	/* Create second keyed data sample for writing */
	instanceB = ShapeTypeExtendedTypeSupport::create_data();
	if (instanceB == NULL) {
		fprintf(stderr, "ShapeTypeExtendedTypeSupport::create_data error\n");
		publisher_shutdown(participant);
		return -1;
    ...
    written multiple times, initialize the key(s) here
    and register the keyed instances prior to writing */

    instanceA_handle = ShapeTypeExtended_writer->register_instance(*instanceA);
	instanceB_handle = ShapeTypeExtended_writer->register_instance(*instanceB);

	// Initialize the non-changing fields of each of the two instances created
	// above and initialize the position values...
	//
	strcpy(instanceA->color, "RED");
	strcpy(instanceB->color, "YELLOW");
	instanceA->shapesize = 25;
	instanceB->shapesize = 25;
	instanceA->x = 50;
	instanceB->x = 150;
	instanceA->y = 10;
	instanceB->y = 10;
    ...
    for (count=0; (sample_count == 0) || (count < sample_count); ++count) {
        ...
        retcode = ShapeTypeExtended_writer->write(*instanceA, instanceA_handle);
        if (retcode != DDS_RETCODE_OK) {
            fprintf(stderr, "write error %d\n", retcode);
        }
		retcode = ShapeTypeExtended_writer->write(*instanceB, instanceB_handle);
		if (retcode != DDS_RETCODE_OK) {
			fprintf(stderr, "write error %d\n", retcode);
		}
		...
		if ((instanceA->y += 10) > 240) instanceA->y = 10;
		if ((instanceB->y += 10) > 240) instanceB->y = 10;
	}
    ...
    */
    ...
    /* Delete data samples */
    retcode = ShapeTypeExtendedTypeSupport::delete_data(instanceA);
    if (retcode != DDS_RETCODE_OK) {
        fprintf(stderr, "ShapeTypeExtendedTypeSupport::delete_data error %d\n", retcode);
    }
	retcode = ShapeTypeExtendedTypeSupport::delete_data(instanceB);
	if (retcode != DDS_RETCODE_OK) {
		fprintf(stderr, "ShapeTypeExtendedTypeSupport::delete_data error %d\n", retcode);
	}
